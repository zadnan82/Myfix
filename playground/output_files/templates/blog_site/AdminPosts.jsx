...
export default function Blog_SiteAdminpostsPage() {
  return (
    <>
  ...
  <img 
    ...
    onLoad={() => {
        ...
        fetch('/api/admin/blog/posts?' + params)
          ...
          .then(data => {
            ...
            if (data.posts && data.posts.length > 0) {
              ...
              container.innerHTML = '<div class="text-center py-12"><span class="text-4xl">📝</span><p class="mt-4 text-gray-500">No posts found</p><button onclick="window.location.href=\'/admin/editor\'" class="mt-4 text-blue-600 hover:text-blue-700">Create your first post</button></div>';
            }
      ...
      window.togglePublish = function(postId, publish) {
        fetch('/api/admin/blog/posts/' + postId + '/publish', {
          method: 'PATCH'
        }).then(response => {
          if (response.ok) {
