...
export class WebSocketService {
  ...
  getWebSocketUrl(endpoint) {
    // Get the current page protocol to determine ws vs wss
    const isSecure = window.location.protocol === 'https:';
    const wsProtocol = isSecure ? 'wss://' : 'ws://';
    
    // Use the configured API base URL and convert it to WebSocket protocol
    let baseUrl = CONFIG.API.BASE_URL.replace('http://', 'ws://').replace('https://', 'wss://');
    
    // Debug logging for URL construction (production enabled for debugging)
    console.log('[WebSocket URL] Original API base:', CONFIG.API.BASE_URL);
    console.log('[WebSocket URL] WebSocket base:', baseUrl);
    console.log('[WebSocket URL] Endpoint:', endpoint);
    ...
    console.log('[WebSocket URL] Final URL:', finalUrl);
  ...
  connectToNotifications(onMessage, onError = null, onClose = null) {
    ...
    const token = storage.get(CONFIG.STORAGE_KEYS.SESSION_TOKEN);
    const endpoint = buildEndpoint('WS_NOTIFICATIONS');
    
    // Debug logging (production enabled for debugging)
    console.log('[WebSocket] Raw endpoint from buildEndpoint:', endpoint);
    console.log('[WebSocket] Token exists:', !!token);
    console.log('[WebSocket] Current location protocol:', window.location.protocol);
    console.log('[WebSocket] Current location host:', window.location.host);
