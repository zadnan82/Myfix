...
export class ApiClient {
  ...
  setAuthToken(token) {
    try {
      if (token) {
        storage.set(CONFIG.STORAGE_KEYS.SESSION_TOKEN, token);
      } else {
    ...
    const requestFn = async () => {
      ...
      try {
        ...
        if (token) {
          ...
          // Debug logging for token issues (only in development or when debug is enabled)
          if (!this.isProduction || import.meta.env.VITE_DEBUG_API === 'true') {
            console.log(`[API] Using token for ${method.toUpperCase()} ${url}:`, token.substring(0, 10) + '...');
          }
        } else {
          // Debug logging for missing token
          if (!this.isProduction || import.meta.env.VITE_DEBUG_API === 'true') {
            console.warn(`[API] No token found for ${method.toUpperCase()} ${url}`);
          }
        }
...
console.log('[API Client] Using base URL:', CONFIG.API.BASE_URL);
...
apiClient.addErrorInterceptor(async (error) => {
  if (error.isAuthError() && error.status === 401) {
    // Clear invalid token using storage service
    try {
      storage.remove(CONFIG.STORAGE_KEYS.SESSION_TOKEN);
