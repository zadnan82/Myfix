...
async def apply_ai_edit(
    ...
    """Apply AI-powered edits (unified file flow or legacy website edits)"""

    try:
        # Unified file edit flow
        if request.filePath and (request.content is not None) and request.prompt:
            ...
            if not result or not isinstance(result.modified_code, str):
                raise HTTPException(
                    status_code=500, detail="Invalid AI response")
            return AIEditResponse(
                success=True,
                ok=True,
                message="Edit applied",
                filePath=request.filePath,
                editedContent=result.modified_code,
                usage=None,
                changes=["Edited via unified flow"],
            )

        # Find the generated website directory (legacy)
        ...
        await notify_preview_update(
            ...
            changes=agent_response.get(
                "changes", [f"Applied: {request.instruction}"]),
...
async def call_agent_system(
    ...
    try:
        ...
        # Handle other common instructions
            ...
            logger.info(
                f"Instruction processed but no specific handler: {instruction}")
...
async def modify_about_page_color(website_dir: str, color: str) -> bool:
    ...
    try:
        about_file = Path(website_dir) / "frontend" / \
...
async def modify_home_page_color(website_dir: str, color: str) -> bool:
    ...
    try:
        home_file = Path(website_dir) / "frontend" / \
