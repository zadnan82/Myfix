...
class Settings(BaseSettings):
    ...
    # Application
    APP_NAME: str = Field(default="SEVDO User API",
                          description="Application name")
    APP_VERSION: str = Field(
        default="2.0.0", description="Application version")
    ...
    RELOAD_ON_CHANGE: bool = Field(
        default=False, description="Reload on file changes")
    DEBUG_ENABLED: bool = Field(default=False, description="Enable debug mode")

    # LLM / GPU settings
    GPU_LLM_BASE_URL: Optional[str] = Field(
        default=None,
        description="Base URL of GPU LLM service (e.g., http://192.168.16.103:8000)",
    )
    LLM_DEFAULT_MODEL_NAME: str = Field(
        default="gpt-oss:20b",
        description="Default model name for LLM editor",
    )
    LLM_DEFAULT_SYSTEM_PROMPT: str = Field(
        default="You are a helpful assistant.",
        description="Default system prompt for LLM editor",
    )
    LLM_EDITOR_ALLOW_PUBLIC: bool = Field(
        default=False,
    ...
    def validate_upload_size(cls, v):
        ...
        if v < 1024:  # 1KB minimum
            raise ValueError(
                "UPLOAD_MAX_SIZE must be at least 1024 bytes (1KB)")
...
try:
    ...
    class FallbackSettings:
        ...
        DB_URL = os.getenv(
            "DB_URL",
            # Prefer docker-compose default password if unset
            f"postgresql://sevdo_user:{os.getenv('POSTGRES_PASSWORD', 'devpassword')}@localhost:5432/{os.getenv('POSTGRES_DB', 'sevdo_db')}",
        )
        ...
        # LLM / GPU defaults
        GPU_LLM_BASE_URL = os.getenv(
            "GPU_LLM_BASE_URL", "http://192.168.16.103:8000")
        LLM_DEFAULT_MODEL_NAME = os.getenv(
            "LLM_DEFAULT_MODEL_NAME", "gpt-oss:20b")
        LLM_DEFAULT_SYSTEM_PROMPT = os.getenv(
