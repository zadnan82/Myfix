# sevdo_frontend/prefabs/blog_post_editor.py
def render_prefab(args, props):
    # Default values
    title = props.get("title", "Blog Post Editor")
    mode = props.get("mode", "create")
    post_id = props.get("postId", "")

    # API configuration
    api_path = props.get("api_path", "/api/admin/blog/posts")
    tags_endpoint = props.get("tags_endpoint", "/api/blog/tags")
            ...
            if nodes:
                for node in nodes:
                    if node.token == "h" and node.args:
                        ...
                        mode = node.args
                    elif node.token == "id" and node.args:
                        post_id = node.args
        except Exception:
            title = args if args else title

    page_title = f"Edit Post" if mode == "edit" else "Create New Post"
...
# Register with token "bpe" (blog post editor)
COMPONENT_TOKEN = "bpe"
