...
async function fetchState(){
  const r = await fetch('/dev/dashboard/state');
  STATE = await r.json();
...
async function selectTask(name, silent){
  SELECTED = name;
  const r = await fetch('/dev/dashboard/task/'+encodeURIComponent(name));
  ...
  document.getElementById('downloadSummary').href = '/dev/dashboard/download/'+encodeURIComponent(d.name)+'/summary';
  document.getElementById('downloadLog').href = '/dev/dashboard/download/'+encodeURIComponent(d.name)+'/full';
  document.getElementById('downloadNewFiles').href = '/dev/dashboard/download/'+encodeURIComponent(d.name)+'/newfiles';
  ...
  if(actions){
    ...
    killBtn.onclick = async ()=>{ await fetch('/dev/dashboard/kill/'+encodeURIComponent(d.name), {method:'POST'}); fetchState(); };
    const delBtn = document.createElement('button');
    delBtn.className='secondary';
    delBtn.id='deleteBtn';
    delBtn.textContent='Delete';
    delBtn.onclick = async ()=>{ await fetch('/dev/dashboard/delete/'+encodeURIComponent(d.name), {method:'POST'}); document.getElementById('detailCard').style.display='none'; fetchState(); };
...
document.getElementById('startBtn').addEventListener('click', async ()=>{
  ...
  if(!txt) return;
  await fetch('/dev/dashboard/add', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({task:txt, workdir: wd, files, model})});
...
document.getElementById('viewLog').addEventListener('click', async ()=>{
  if(!SELECTED) return;
  const r = await fetch('/dev/dashboard/log/'+encodeURIComponent(SELECTED));
  if(r.ok){
...
document.getElementById('retryBtn').addEventListener('click', async ()=>{
  if(!SELECTED) return;
  const r = await fetch('/dev/dashboard/task/'+encodeURIComponent(SELECTED));
  if(!r.ok) return;
  const d = await r.json();
  await fetch('/dev/dashboard/add', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({task:d.task_text || '', workdir: d.workdir || ''})});
  fetchState();
});

document.getElementById('clearAllBtn').addEventListener('click', async ()=>{
  if(!confirm('Delete all tasks and outputs?')) return;
  await fetch('/dev/dashboard/clear', {method:'POST'});
...
document.getElementById('runSolveSubtask').addEventListener('click', async ()=>{
  ...
  if(!subtask) return;
  const r = await fetch('/dev/dashboard/api/test-solve-subtask', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({subtask, model})});
...
document.getElementById('runRagRace').addEventListener('click', async ()=>{
  ...
  try{
    const r = await fetch('/dev/dashboard/api/rag-race', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({task, models})});
    if(!r.ok){
      const txt = await r.text();
      el.textContent = 'Error: ' + txt;
      return;
...
@app.route('/dev/dashboard/add', methods=['POST'])
def add():
...
@app.route('/dev/dashboard/task/<name>')
...
@app.route('/dev/dashboard/log/<name>')
...
@app.route('/dev/dashboard/download/<name>/<kind>')
...
@app.route('/dev/dashboard/api/test-solve-subtask', methods=['POST'])
...
@app.route('/dev/dashboard/api/rag-race', methods=['POST'])
def api_rag_race():
    js = request.get_json(force=True, silent=True) or {}
...
@app.route('/dev/dashboard/kill/<name>', methods=['POST'])
def kill(name: str):
    safe_name = os.path.basename(name)
    pid_file = os.path.join(OUTPUT_DIR, f"{safe_name}.pid")
...
@app.route('/dev/dashboard/delete/<name>', methods=['POST'])
...
@app.route('/dev/dashboard/clear', methods=['POST'])
def clear_all():
